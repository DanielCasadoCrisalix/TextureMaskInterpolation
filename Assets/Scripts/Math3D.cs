...
public class Math3D
{
    ...
    public static Vector2 GetPointOnSpline(float percentage, Vector2[] cPoints)
    {
        ...
        if (cPoints.Length >= 4)
        {
            ...
            Vector2 result = .5f * (2f * p1 + (-p0 + p2) * t + (2f * p0 - 5f * p1 + 4f * p2 - p3) * (t * t) +
    ...
    public static float[] GetLineSplineIntersections(Vector2[] linePoints, Vector2[] cPoints)
    {
        ...
        for (int i = 0; i < numSections; i++)
        {
            ...
            float C = (linePoints[0].x - linePoints[1].x) * linePoints[0].y +
    ...
    public static bool ProjectPointOnLineSegmentCheck(Vector3 linePoint1, Vector3 linePoint2, Vector3 point,
        out Vector2 pointOverLine)
    {    
        
        Vector3 vector = linePoint2 - linePoint1;

        pointOverLine = ProjectPointOnLine(linePoint1, vector.normalized, point);

        return PointOverSegmentCheck(linePoint1, linePoint2, pointOverLine);
    }

    private static bool PointOverSegmentCheck(Vector3 linePoint1, Vector3 linePoint2, Vector3 point)
    {
